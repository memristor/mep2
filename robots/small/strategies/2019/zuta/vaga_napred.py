...
def leave_front():
	r.speed(120)
	if not pathfind(169+30+5, 187): #dodato +30 zbog precke u sredini da ne zakaci slucajno
		return False
	#nosi ga na vagu
	r.speed(100)
	...
	r.goto(200-13-13-5-10+1,250-20,1) #idi preko celog terena do ispred vage !
	p4 = napredp.picked()		# Uhvatio ??????

	@_do						
	def _():
		if(p4.val):
			addpts(24) #gold nosi 24 boda na vagi
			print("Dodao bodove ------------------------------")
		else:
	...
	with disabled('collision'):
		...
		_on('motion:stuck', f)
		x,y=coord('vaga') # originalno -200+13
		r.goto(x-10-1,y)
		r.speed(50)
		r.absrot(90)
		r.forward(100)
...
def leave_back():
	...
	x,y=coord('vaga') # originalno -200+13
	r.goto(x-11,y,-1)
	#r.goto(200-20-5-4-5-5-5,470,-1)	#Pred vagom KRITICNO
	#r.forward(170)
	r.absrot(270)
	r.speed(80)
	r.forward(-100)
...
'''def leave_front():
	...
	def _():
		if(p3.val or True):
			...
			pump(2, 0)
			nazgold(0)	# Nije uhvatio, zavrsi
			r.goto(168,170-40) #ide pred haos zonu
			_task_done()
			return
	...
	r.goto(200-13-13-5-10,250-20,1)
	...
	with disabled('collision'):
		...
		r.conf_set('enable_stuck', 1)
		_on('motion:stuck', f)
		r.goto(*coord('vaga'),1) #Pozicija na koju ostavlja goldenium
		r.speed(50)
		r.absrot(90)
		r.forward(100)
		r.conf_set('enable_stuck', 0)
