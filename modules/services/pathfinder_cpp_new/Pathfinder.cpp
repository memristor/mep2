...
Path Pathfinder::Search(Point start, Point end) {
	...
	if(it != m_polygons.end()) {
		...
		for(auto &poly : m_polygons) {
			auto &nodes = poly.second.GetNodes();
			//std::cout << "poly " << (cnt++) << "\n";
			for(int i=0; i < nodes.size(); i++) {
				if(it->second.IsLineSegmentIntersectingPolygon(LineSegment(nodes[i]->point, nodes[(i+1) % nodes.size()]->point))) {
					polys.push_back(&poly.second);
					break;
				}
			}
		}
		//std::cout << "joining " << polys.size() << "\n";
		Polygon poly = Polygon::JoinPolygons(polys,3);
