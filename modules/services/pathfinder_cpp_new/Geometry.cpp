...
LineSegment LineSegment::Extend(int offset) {
	Vec2F norm = vec.Normalized();
	LineSegment ls(*this);
	Point a = this->a;
	Point b = this->b;
	
	a.MoveInDirection(norm.x, norm.y, -offset);
	b.MoveInDirection(norm.x, norm.y, offset);
	
	return LineSegment(a,b);
}

Point LineSegment::OffsetPoint(int p, int offset) {
	Vec2F norm = vec.Normalized();
	Point pt;
	if(p == 0) {
		pt = this->a;
	...
	pt.MoveInDirection(norm.x, norm.y, offset);
...
bool Polygon::IsLineSegmentIntersectingPolygon(const LineSegment ls) const {
	...
	for(unsigned int i=0; i < nodes.size(); i++)
	{
		...
		const Point &edgeP3 = nodes[i+2 >= nodes.size() ? (i+2)%nodes.size() : i+2]->point;
		...
		LineSegment ls1(edgeP1, edgeP2);
		LineSegment ls2(edgeP2, edgeP3);
		...
		int d=10;
		// LineSegment ls3( ls1.OffsetPoint(1, -d), ls2.OffsetPoint(0, +d) );
		// dbg(stage.Report("line", ls3, 0xffE03434);)
...
Polygon Polygon::JoinPolygons(std::vector<Polygon*> polys, int inflate) {
	clip::Clipper c;
	...
	if(inflate) {
		clip::ClipperOffset c_ofs;
		c_ofs.AddPath(solutions[0], clip::jtSquare, clip::etClosedPolygon);
		c_ofs.Execute(solutions, inflate);
	}
