...
def p32(x): return struct.pack('>I', to_uint32(x))
def p32l(x): return struct.pack('<I', to_uint32(x))
def p16(x): return struct.pack('>H', to_uint16(x))
def p16l(x): return struct.pack('<H', to_uint16(x))
# loading
...
def lu32(x, idx): return struct.unpack('>I', x[idx:idx+4])[0]
def lu32l(x, idx): return struct.unpack('<I', x[idx:idx+4])[0]
